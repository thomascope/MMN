...
inversion_prefix = 'raedfff';

raw_prefix = '8LFP_LOR_raedfff';
...
for subj = 1:size(all_subjs,1)
    raw_suffix = strsplit(filenames{subj},inversion_prefix);
    raw_suffix = raw_suffix{2}; %get the participant i.d.
    if exist([pathstem_evoked dirnames_inv{all_subjs(subj,1)} '/' raw_prefix raw_suffix],'file')
        unaveragedfile = [pathstem_evoked dirnames_inv{all_subjs(subj,1)} '/' raw_prefix raw_suffix];
    else
        error([pathstem_evoked dirnames_inv{all_subjs(subj,1)} '/' raw_prefix raw_suffix  ' does not contain the LFP data file'])
    end
    ...
    if exist([this_outdir 's' num2str(all_subjs(subj,:)) '_grangerdata_highfreq_averagesubtracted_100_' num2str(start_times) '_' num2str(end_times) '_z' num2str(1) '.mat'],'file')
        ...
        parallel_MMN_coherence_granger_resume(unaveragedfile,[],this_outdir,all_subjs(subj,:),start_times,end_times,fft_method,method,uptohere)
    else
        disp(['job started on worker ' num2str(subj)])
        parallel_MMN_coherence_granger(unaveragedfile,[],this_outdir,all_subjs(subj,:),start_times,end_times,fft_method,method)
    end
end
