function maxfilter_this_participant(rawdatapath,subjfolder,outfilename)
...
if max(size(rawdatapath))==1
    ...
    for this_file = 1:max(size(rawdatapath))
        ...
        S.dataset = fullfile(datapath,sprintf('%s_trans1st.fif',raw_stem));
        %S.channels = 'all';
        S.outfile  = ['tempdata_' num2str(this_file)];
        premerged_files{thisfile} = outfile;
        if ~exist(sprintf('f%s.mat',subjfolder), 'file')
            D = spm_eeg_convert(S);
        else
            D = spm_eeg_load(sprintf('%s.mat',subjfolder));
...
disp(['Files converted and merged into ' D.fname])
