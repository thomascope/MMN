function O = DCMgraphTA_cmc_COMBINED_HP(BMA,varargin)


% setup:
V = varpairs2structTA(varargin{:});
if ~isfield(V,'plot_field'), V.plot_field = 'A'; end % 'A' for baseline trial input, 'A+B' for comparison trial input, 'B' for difference
if ~isfield(V,'DMn'), V.DMn = 1; end % design matrix column number
if ~isfield(V,'nodes'), V.nodes = {'left A1', 'left STG', 'left IFG', 'left IPC', 'right A1',  'right STG', 'right IFG',  'right IPC'}; end % {'L1' 'L2' 'L3' 'L4' 'R1' 'R2' 'R3' 'R4'};
if ~isfield(V,'pops'), V.pops = {'Fwd out' 'Bck out' 'Fwd in' 'Bck in'}; end
if ~isfield(V,'diffTF'), V.diffTF = 1; end
if ~isfield(V,'coldiffTF'), V.coldiffTF = 1; end
if ~isfield(V,'clim'), V.clim = []; end
if ~isfield(V,'conn_for'), V.conn_for = [1 3]; end % i.e. sp->ss ; sp->dp
if ~isfield(V,'conn_bac'), V.conn_bac = [2 4]; end % i.e. dp->sp ; dp->ii
if ~isfield(V,'conn_DA'), V.conn_DA = [1 1; 2 2; 3 3; 4 4]; end % i.e. dp->sp ; dp->ii
if ~isfield(V,'conn_for_type'), V.conn_for_type = [1]; end % i.e. the two conn_for are positive (use zero for modulatory)
if ~isfield(V,'conn_bac_type'), V.conn_bac_type = [-1]; end % i.e. the two conn_bac are negative (use zero for modulatory)
...
for kb = 1:length(V.DMn)
    ...
    if V.signifTF
        HVAL = full(BMA.Pp(:,V.DMn(kb))) >=.95;
        hvala = {zeros(nn,nn) zeros(nn,nn)};
        hvalan = {zeros(nn,nn) zeros(nn,nn)};
        hvalm = zeros(nn,nn);
        hvalc = zeros(nn,1);
    end
    aval = {zeros(nn,nn) zeros(nn,nn)};
    anval = {zeros(nn,nn) zeros(nn,nn)};
    mval = zeros(nn,nn);
    cval = zeros(nn,1);
    for k = 1:length(BMA.Pnames)
        ...
        if length(s)==2
            if strcmp(s{2},BMA.Pnames{k}(1)) && ~strcmp([s{2} 'N'],BMA.Pnames{k}(1:2))
                ...
                aval{loca(k,1)}(loca(k,2),loca(k,3)) = aval{loca(k,1)}(loca(k,2),loca(k,3)) + BMA.Ep(k,V.DMn(kb));
            end
    ...
    %if strcmp('B',V.plot_field)
        aval_temp{2} = triu(aval{1});
        aval_temp{1} = tril(aval{1});
        anval{2} = triu(anval{1});
        anval{1} = tril(anval{1});
    %end
    ...
    for kc = 1:size(V.conn_for,1) % forward AMPA connections
        ...
        for ki = 1:nn
            for kj = 1:nn
                if V.signifTF
                    CG1{1}((ki*np)-np+1+c(2),(kj*np)-np+1+c(1)) = aval{1}(ki,kj) .* hvala{1}(ki,kj);
                else CG1{1}((ki*np)-np+1+c(2),(kj*np)-np+1+c(1)) = aval{1}(ki,kj);
                end
            end
        end
    end
    for kc = 1:size(V.conn_bac,1) % backward AMPA connections
        ...
        for ki = 1:nn
            for kj = 1:nn
                if V.signifTF
                    CG1{2}((ki*np)-np+1+c(2),(kj*np)-np+1+c(1)) = aval{2}(ki,kj) .* hvala{2}(ki,kj);
                else CG1{2}((ki*np)-np+1+c(2),(kj*np)-np+1+c(1)) = aval{2}(ki,kj);
                end
            end
        end
    end
    ...
    tit = {V.plot_field};%tit = {'AMPA'};% 'top-down-DA'};
