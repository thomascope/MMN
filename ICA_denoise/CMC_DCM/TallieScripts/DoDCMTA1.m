...
%F = FilesTA;
F{1} = 'your/path/to/the/file/extracted_nodes.mat'

%% THIS VERSION WON'T WORK IN PARALLEL
% open pool:
...
for KK = 1
    ...
    xU.X                = [0 1; 1 0];                                           % a design matrix for trial comparisons
    xU.name             = {'std' 'dev'};                                        % names for the rows of the design matrix
%%
    for k = 1:length(F)
        ...
        xY.Dfile            = fn;
        xY.modality         = 'LFP';                                                % assuming fn contains reconstructed sources of your ROIs
        xY.name             = arrayfun(@(x) x.label,d.D.channels,'Uni',0);          % names of your ROIs (e.g. leftSTG, rightA1, etc)
        xY.Ic               = 1:size(d.D.data,1);
        xY.Time             = t;
        xY.dt               = 1/Fs;
        ...
        DCM.xY              = xY;
        DCM.Sname           = xY.name';
        DCM.Lpos            = double.empty(3,0);                                    % I believe these are only needed if you are doing ECD and haven't inputted LFPs
        ...
        %DCM.symmetricTF = 0; % custom addition - TA mod of alex CAN'T USE DUE TO SPM_VEC
