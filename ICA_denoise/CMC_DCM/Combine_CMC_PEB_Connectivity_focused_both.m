...
for this_focus = 1:length(PEB_focuses)
    ...
    for this_contrast = 2:size(template_PEB.M.X,2)
        ...
        for this_difference = 1:length(these_differences)
            ...
            involves_controls(end+1) = template_PEB.M.X(1,this_contrast)~=0; %Assumes controls are first
            all_these_contrasts(end+1) = this_contrast;
            ...
            if strcmp(PEB_focus,'A')
                main_effect_significances(end+1,1:length(all_granger_data)) = zeros(1,length(all_granger_data));
                permutation_main_effect_significances(end+1,1:length(all_granger_data)) = zeros(1,length(all_granger_data));
            else
                interaction_significances(end+1,1:length(all_granger_data)) = zeros(1,length(all_granger_data));
                permutation_interaction_significances(end+1,1:length(all_granger_data)) = zeros(1,length(all_granger_data));
            end
                    ...
                    if strcmp(PEB_focus,'A')
                        ...
                        for this_band = 1:size(Frequency_bands,1)
                            ...
                            if pval < post_thresh || permutation_p < post_thresh
                                if pval < post_thresh
                                    main_effect_significances(end,this_measure) = 1;
                                end
                ...
                elseif this_connectivity_contrast == 3
                    ...
                    if strcmp(PEB_focus,'B')
                        ...
                        for this_band = 1:size(Frequency_bands,1)
                            ...
                            if pval < post_thresh || permutation_p < post_thresh
                                if pval < post_thresh
                                    interaction_significances(end,this_measure) = 1;
                                end
                                if permutation_p < post_thresh
...
if do_nonsigs
    ...
    for this_focus = 1:length(PEB_focuses)
        ...
        for this_loop = 1:(length(BMA.Pnames)*length(all_granger_data)*(size(template_PEB.M.X,2)-1))
            ...
            if strcmp(PEB_focus,'A')
                overall_main_effect_significances(this_loop) = 0;
                ...
                for this_band = 1:size(Frequency_bands,1)
                    ...
                    if pval < post_thresh || permutation_p < post_thresh
                        if pval < post_thresh
                            overall_main_effect_significances(this_loop) = 1;
                        end
                        if permutation_p < post_thresh
            ...
            elseif strcmp(PEB_focus,'B')
                ...
                for this_band = 1:size(Frequency_bands,1)
                    ...
                    if pval < post_thresh || permutation_p < post_thresh
                        if pval < post_thresh
                            overall_interaction_significances(this_loop) = 1;
                        end
                        if permutation_p < post_thresh
