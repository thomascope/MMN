...
%         end
    if any(any(isnan(D.DCM.H{1}))) ||  any(any(isnan(S.DCM.H{1}))) %Debugging of NaNs
...
for i = 1:8
    ...
    hold on
    plot(0:4:400, squeeze(nanmean(all_STDs_data(:,i,:),3)))
    title(D.DCM.xY.name{i})
end
for i = 1:8
    subplot(4,4,i+8)
    plot(0:4:400, squeeze(nanmean(all_DVTs_modelled(:,i,:),3)))
    hold on
    plot(0:4:400, squeeze(nanmean(all_DVTs_data(:,i,:),3)))
    title(S.DCM.xY.name{i})
end