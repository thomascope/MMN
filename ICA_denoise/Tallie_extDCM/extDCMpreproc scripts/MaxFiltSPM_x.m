function MaxFiltSPM_x(xlsx,rawpathstem,pathstem,do_subjects)
%
% Maxfilter final edit by TEC, after edited by ES, based on script by Jason Taylor
% Tidied superficially a bit by TA
%
% This function maxfilters raw MEEG data (.fif) and converts it into SPM
% format (.mat). To use it you will need a collated list of files you want
% to run it on, locations for where you want to save them, and noted
% channels that were bad on the day of recording. This information is all
% held in very simple .xlsx, specified as stated below in the 'xlsx'
% input variable.
%
% INPUTS:
...
% rawpathstem = outer path to the raw datafiles (e.g. '/megdata/cbu/ftdrug/')
% pathstem    = outer path to the maxfiltered datafiles (e.g. '/imaging/group/rowelab/MEM/resting_closed/maxfilt/')
% do_subjects = which subjects you'd like to do from your xls file - i.e.
%               which rows.
%
% PLEASE NOTE! A word of caution: I have found maxfilter to be inexplicably
% fickle. Sometimes it doesn't run for some subjects, but upon repeating it
% works fine. Just to save unnecessary debugging time - you can always try
% to run it again :)
%

% Import meta:
O = MaxFiltSPM_batchinfo_XLSXinput(xlsx,rawpathstem,pathstem);
subjects = O.subjects;
dates = O.dates;
blocksin = O.blocksin;
blocksout = O.blocksout;
badchannels = O.badchannels;
badeeg = O.badeeg;
...
for ss = do_subjects
    ...
    for bb = 1:nr_bls
        try rawpath = fullfile( rawpathstem, subjects{ss}.oldname, subject{ss}{2} );
        catch err
            error(rawpath)
        end
    ...
    for ss = do_subjects
        ...
        for bb = 1:nr_bls
            ...
            if exist(outfname3,'file')
                ...
                skipint = '0 20';
                mfcmd2 = ['/neuro/bin/util//neuro/bin/util/maxfilter -f ' [rawfname] ' -o ' [outfname1],' -autobad 20 -v | tee ' [logfname1]];
                fprintf(1, '\n\n%s\n\n', mfcmd2);
                if exist(outfname2,'file')
                    ...
                    badcmd = ['cat ' [logfname1] ' | sed -n ''/Static/p'' | cut -f 5- -d '' '' > ' [badfname]];
                    ...
                    try x = dlmread([badfname],' ');
                    catch err % For case where no bad channels
                        x = 0;
                    end
        ...
        % TA:
                ...
                if bb>1
                    ...
                    fprintf(1, '%s\n', mfcmd4);
                    eval([' ! ' mfcmd4 ])
                else b1file = outfname2;     % file used for future "trans"
