function spm_eeg_inv_groupTA(S,sMRI)
...
for i = 1:Ns
    ...
    D{i}                 = spm_eeg_load(S{i}); % (deblank(S(i,:)));
    D{i}.val             = 1;
...
for i = NS
    ...
    %======================================================================
    D{i} = spm_eeg_inv_mesh_ui(D{i}, 1, sMRI{i}, Msize);
...
%==========================================================================
inverse = spm_eeg_inv_custom_uiTA(D{1});
...
if strcmp(mod, 'Multimodal')
    ...
    inverse.modality  = 'MEGPLANAR'; % list(selection);
...
for i = NS
    ...
    % Forward model
    %----------------------------------------------------------------------
    D{i} = spm_eeg_inv_datareg_uiTA(D{i}, 1);
    try
        D{i}.inv{1}.forward.voltype = vltype;
        D{i}   = spm_eeg_inv_forward_uiTA(D{i});
    catch
        D{i}   = spm_eeg_inv_forward_uiTA(D{i});
