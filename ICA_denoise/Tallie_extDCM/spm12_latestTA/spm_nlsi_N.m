...
for ip = 1:M.Nmax
    ...
    for ig = 1:Gmax
        ...
        %------------------------------------------------------------------
        ep    = Vp'*(spm_vec(Ep) - spm_vec(pE)); % TA weighting in here???
    ...
    if exist('Fsi', 'var')
        ...
        try
            %plot(yt,x(:,[37:end]),'-k')
            for k = [1:3:36], plot(yt,x(:,k),'Color',clr(k,:)), end
            xlabel('time (ms)')
            set(gca,'XLim',[yt(1) yt(end)]), ylim([-90 50])
        catch
        ...
        subplot(4,2,2), cla, hold on
        try
            %plot(yt,x(:,[37:end]),'-k')
            for k = [2:3:36], plot(yt,x(:,k),'Color',clr(k,:)), end
            xlabel('time (ms)')
            set(gca,'XLim',[yt(1) yt(end)]), ylim([-90 50])
        catch
            plot(x), spm_axis tight
        ...
        try
            %plot(yt,x(:,[37:end]),'-k')
            for k = [3:3:36], plot(yt,x(:,k),'Color',clr(k,:)), end
            xlabel('time (ms)')
            set(gca,'XLim',[yt(1) yt(end)]), ylim([-90 50])
        catch
            plot(x), spm_axis tight
        ...
        clr2 = jet(216);
        x_sign = cat(2,ones(1,6*6),ones(1,6*6),-ones(1,6*6),-ones(1,6*6),-ones(1,6*6),ones(1,6*6));
        try
            for k = 37:252, plot(yt,x(:,k)*x_sign,'Color',clr2(k-36,:)), end
%             for k = [3:3:36], plot(yt,x(:,k),'Color',clr(k,:)), end
%             xlabel('time (ms)')
%             set(gca,'XLim',[yt(1) yt(end)])
        ...
        subplot(4,2,[5 7]), cla, hold on
        ype = spm_unvec(ey,yp);
        for k = 1:size(yp,2)
            plot(yt,yp(:,k),'Color',clr((k*6)-5,:),'LineWidth',2.5)
            plot(yt,yp(:,k) + ype(:,k),'Color',clr((k*6)-5,:))
        end
        xlabel('time (ms)')
        set(gca,'XLim',[yt(1) yt(end)])
...
%     R.p{8}(:,:,ip) = Ep.H(:,:,2);
%     R.p{9}(:,:,ip) = Ep.H(:,:,3);
%     R.p{10}(:,:,ip) = [Ep.C Ep.CV' [Ep.D';Ep.R';Ep.S]];
%     R.p{11}(:,:,ip) = Ep.G;
%     R.p{12}(:,:,ip) = Ep.T;
%     R.p{13}(:,:,ip) = full(Eg.J);
%     R.p{14}(:,:,ip) = Eg.L;
