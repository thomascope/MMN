...
% 
% s = strsplit(lower(char(M.f)),'_');
% s = strcat(s{1},'_',s{2},'_',s{3});
% switch s
%     
...
%         M.u   = sparse(ns,1);
%         dnx   = 0;
%         for i = 1:32 % TA reduced from 128 to save time
%             
%             % solve under locally linear assumptions
%             %--------------------------------------------------------------
%             [f,dfdx] = feval(M.f,M.x,M.u,P,M);
%             dx       = - dfdx\f; % note backslash - solve system of linear equations for x
%             
%             % regularise
%             %--------------------------------------------------------------
%             ndx   = norm(dx,Inf);
