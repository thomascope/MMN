function [DCM,dipfit] = spm_dcm_erp_nonx(DCM)
...
if DATA
    if DCM.options.DoData == 1;
        DCM = spm_dcm_erp_data(DCM); % sorting out what you'll be working with from your data - nothing model specific
    elseif DCM.options.DoData == -1;
        DCM = spm_dcm_erp_dataIFGYinvert(DCM); % sorting out what you'll be working with from your data - nothing model specific
    elseif DCM.options.DoData == 2;
        DCM = spm_dcm_erp_dataKaiser(DCM); % sorting out what you'll be working with from your data - nothing model specific
    elseif DCM.options.DoData == 3;
        DCM = spm_dcm_erp_dataTukey(DCM); % sorting out what you'll be working with from your data - nothing model specific
    elseif DCM.options.DoData == 5;
        DCM = spm_dcm_erp_dataTukey5(DCM);
    end
...
try CUS = DCM.CUSTOM;
    ...
    try        M.genQ = CUS.genQ;   fprintf('\ncustom state equations %s',CUS.f); catch err, M.genQ = 'spm_gen_Q'; end
    try       pE.CV= CUS.pE.CV;fprintf('\ncustom priors for CV');           end
    try       pC.CV= CUS.pC.CV;           end
    try       pE.G = CUS.pE.G;fprintf('\ncustom priors for intrinsics');    end
    try       pC.G = CUS.pC.G;                                              end
    try       pE.H = CUS.pE.H;fprintf('\ncustom priors for intrinsics H, presumably in cmm_NMDA');    end
    try       pC.H = CUS.pC.H;                                              end
    ...
    try       M.fu = CUS.fu;  fprintf('\ncustom thalamic input %s',CUS.fu); end
    try       M.int = CUS.M.int; fprint('\changing intrinsics for each trial\n'), end
    try       G = CUS.G; catch err, G = 'spm_lx_erp';      end
end
...
%--------------------------------------------------------------------------
[x,f,h] = spm_dcm_x_neural(pE,model,M.f);
...
% re-intialise states
%--------------------------------------------------------------------------
M.x    = spm_dcm_neural_x_ignored(pE,M);
