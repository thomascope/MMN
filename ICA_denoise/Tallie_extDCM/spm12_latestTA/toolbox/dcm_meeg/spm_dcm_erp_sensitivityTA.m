function DCM = spm_dcm_erp_sensitivityTA(DCM,C)
...
L   = feval(DCM.M.G, Qg, DCM.M);                 % get gain matrix y = G*x % spm_lx_erp MAY BE USING SYMMETRICAL ASSUMPTIONS % kron of gain-of-each-source and population-contribution
...
j     = logical(j);
for i = 1:numel(x)
    DCM.K{i} = x{i} - x0;                   % centre on expansion point
    %DCM.x{i} = DCM.x{i};
    DCM.H{i} = DCM.M.R*DCM.K{i}*L'*DCM.M.U;             % prediction (sensor space)
    DCM.R{i} = DCM.M.R*DCM.xY.y{i}*M.U - DCM.H{i};      % residuals  (sensor space)
end
