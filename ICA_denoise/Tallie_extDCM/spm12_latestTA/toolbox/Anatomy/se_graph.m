function [Y,y,beta,Bcov] = se_graph(xSPM,SPM,xyz)
...
%--------------------------------------------------------------------------
Fgraph = figure(...
    'Tag','SATB',                             'Position',[25 475 550 400].*spm('WinScale'),...
    'Resize','off',                           'MenuBar','figure',...
    'Color',[.9 .9 .9],                       'ColorMap',gray(64),...
...
XYZ = inv(SPM.xVOL.M)*[xyz];
XYZ = XYZ(1:3,:);       % in voxel

xyz = round(xyz(1:3));  % in mm
