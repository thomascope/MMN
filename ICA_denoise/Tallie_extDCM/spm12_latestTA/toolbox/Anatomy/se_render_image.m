...
if nargin < 1
	...
	for i = 1:num,
        ...
        if nargin < 1
            ...
            k   = spm_input(['Extent threshold (0 for none)'],'+0','r',0,1);
        else
            Vi = spm_vol(imgName);
            pm = 1;
        end
...
try
    ...
    [rendfile, sts] = spm_select(1,'any','Render file'); % .mat or .gii file
    if ~sts, return; end
    end
catch
    [rendfile, sts] = spm_select(1,'any','Render file'); % .mat or .gii file
...
for j=1:length(dat),
    ...
    for i=1:length(rend),
        ...
        if ~isempty(d2)
            ...
            else, msk = find(xyz(3,:) < (z1+35) & xyz(3,:) > (z1-1)); end
        else
            msk = [];
        end
