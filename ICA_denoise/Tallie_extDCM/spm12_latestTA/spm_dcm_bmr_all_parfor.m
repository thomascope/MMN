function [DCM,BMR,BMA] = spm_dcm_bmr_all_parfor(DCM,field)
...
while GS
    ...
    parfor i = 1:size(K,1)
        ...
        G(i) = spm_log_evidence(qE,qC,pE,pC,rE,rC); % evidence for every combination of the 8/16 pars with the least evidence
    end
    ...
    %======================================================================
    [z,i]  = max(p); % find the least probable of the 256/65000 of the 8/16 combos ...
    C(k(K(i,:))) = 0; % ... and remove it
...
pE    = spm_vec(pE);
BMA   = {};
Gmax  = max(G); % at this point, G is the evidence of each 8-par combo from those pars with the least evidence after all pars with least evidence are removed
F = cell(length(K),1); disp('PARFOR 2')
parfor i = 1:length(K)
    if G(i) > (Gmax - 8) % if the free energy is significant
        ...
        [F{i},Ep,Cp]    = spm_log_evidence_reduce(qE,qC,pE,pC,rE,rC); % finds reduced posteriors for every combo of pars with sifgnificant relative free energy
        BMA{i} = struct('Ep',Ep,'Cp',Cp,'F',F{i});
    end
end
disp('PARFOR 2 END')
F = F{end};
ie = find(cellfun(@isempty,BMA));
BMA(ie) = [];

BMR.bma = BMA;
BMA     = spm_dcm_bma_parfor(BMA);
Ep      = BMA.Ep;
Cp      = BMA.Cp;
