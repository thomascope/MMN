function DCM = spm_nlsi_NplusTAcells(DCM)
...
for ip = 1:M.Nmax
    ...
    %----------------------------------------------------------------------
        ...
        for k1 = [1:3:36], plot(1:size(R.x,1),squeeze(R.x(:,k1,k)),'Color',clr(k1,:)), end
        xlabel('time (ms)'), axis tight, ylim([-90 10]), grid on
        title('IFG pop voltages')
        subplot(3,7,[17 18]), hold on
        for k1 = [2:3:36], plot(1:size(R.x,1),squeeze(R.x(:,k1,k)),'Color',clr(k1,:)), end
        xlabel('time (ms)'), axis tight, ylim([-90 10]), grid on
        title('STG pop voltages')
        subplot(3,7,[19 20]), hold on
        for k1 = [3:3:36], plot(1:size(R.x,1),squeeze(R.x(:,k1,k)),'Color',clr(k1,:)), end
        xlabel('time (ms)'), axis tight, ylim([-90 10]), grid on
...
%             if DCM.options.han
%                 plot(1:size(R.yp,1),(R.yp(:,k1,k)-nanmean(R.yp(:,k1,k),1)).*[(tukeywin(size(R.yp,1)/2,.3)+(1/3))/(4/3)';(tukeywin(size(R.yp,1)/2,.3)+(1/3))/(4/3)'],'Color',clr((k1*6)-5,:),'LineWidth',2.5)
%             else plot(1:size(R.yp,1),(R.yp(:,k1,k)-nanmean(R.yp(:,k1,k),1)),'Color',clr((k1*6)-5,:),'LineWidth',2.5)
%             end % [hanning(size(R.yp,1)/2)' hanning(size(R.yp,1)/2)']'
%             plot(1:size(R.y,1),R.y(:,k1,k),'Color',clr((k1*6)-5,:))
%         end
%         axis tight, xlabel('time (ms)'), title('data LFP & predicted LFP'), grid on
%         legend({'yp LIFG' 'y LIFG' 'yp LSTG' 'y LSTG' 'yp LAud' 'y LAud' ...
%                 'yp RIFG' 'y RIFG' 'yp RSTG' 'y RSTG'  'yp RAud' 'y RAud'},...
%                 'FontSize',6,'Position',[0.92 0.44 0.05 0.14])
