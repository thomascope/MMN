...
for i=1:nbchunks
    ...
    for j=1:nScan
        ...
        %cmask(cmask) = Y(j,cmask) > xM.TH(j);      %-Threshold (& NaN) mask % TA removed 
...
%     % TA - TEMPORARY - NEEDS REMOVING:
%     for k = 1:nScan
%         tmp = zeros(size(mask));
%         tmp(cmask) = Y(k,:);
%         tmp = tmp - nanmean(tmp,2) + nanmean(nanmean(tmp,2));
%         tmp = imfilter(tmp,fspecial('disk',4));
%         Y(k,:) = reshape(tmp,[],1);
%     end
    % TA added this section: -- %
    nsubs = ceil(sqrt(nScan));
    %fm; for k = 1:nScan, subplot(nsubs,nsubs,k), te = ones(size(mask)); te(cmask) = Y(k,:); imagesc(te(:,:,10)), set(gca,'Visible','off'), caxis([-2 2]), drawnow, end
    for k = 1:nScan
        te = zeros(size(mask));
        [~,Y(k,:)] = interp_badTA(te,cmask,Y(k,:));
    end
    %fm; for k = 1:nScan, subplot(nsubs,nsubs,k), te = ones(size(mask)); te(cmask) = Y(k,:); imagesc(te(:,:,10)), set(gca,'Visible','off'), caxis([-2 2]), drawnow, end
    % -- -- %
