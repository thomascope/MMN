function [inverse] = spm_eeg_inv_custom_uiTA(D)
...
%==========================================================================
inverse.type = 'GS';
if 1 % spm_input('Model','+1','b',{'Standard|Custom'},[0 1],1)
    
    % Search strategy
    %--------------------------------------------------------------------------
    type         = {'EBB'}; % spm_input('Model inversion','+1','GS|COH|IID|EBB',{'GS','LOR','IID', 'EBB'},1);
    inverse.type = type{1};
    
    % Time window of interest
    %----------------------------------------------------------------------
    woi          = [-100 400]; % spm_input('Time-window (ms)','+1','r',woi);
    inverse.woi  = fix([min(woi) max(woi)]);
    
    % Hanning
    %----------------------------------------------------------------------
    inverse.Han  = 1; % spm_input('PST Hanning','+1','yes|no',[1 0],1);
 
    % High-pass filter
    %----------------------------------------------------------------------
    inverse.lpf  = 0; % spm_input('High-pass (Hz)','+1','0|1|8|16',[0 1 8 16],q);
    
    % Low-pass filter
    %----------------------------------------------------------------------
    inverse.hpf  = 48; % spm_input('Low-pass (Hz)','+1','48|128|256',[48 128 256],q);
    
    % Other source priors (eg from fMRI)
    %----------------------------------------------------------------------
    if 0 % spm_input('Source priors','+1','no|yes',[0 1],1)
    ...
    %----------------------------------------------------------------------
    switch 0 % spm_input('Restrict solutions','+1','no|roi|mask',[0, 1, 2],1)
        case 1
