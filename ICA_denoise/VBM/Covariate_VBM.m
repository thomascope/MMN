function Covariate_VBM(Participant,all_strengths_VBM,metric,from,to)
...
stats_folder = {[outdir 'VBM_stats/factorial_full_group_vbm_TIVnormalised_agecovaried_unsmoothedmask_grouped']};
...
inputs{9, 1} = all_strengths_VBM;
inputs{10, 1} = {'control_majority_unsmoothed_mask_c1_thr0.05_cons0.8.img'};

if ~exist(char(inputs{10, 1}),'file')
...
nrun = 1;
jobfile = {'/imaging/mlr/users/tc02/vespa/scans/PNFA_VBM/tom/VBM_batch_covariate_MCI_TIV_age_nocontrols.m'};
jobs = repmat(jobfile, 1, nrun);
inputs = cell(9, nrun);

stats_folder = {[outdir 'VBM_stats/factorial_full_group_vbm_TIVnormalised_agecovaried_unsmoothedmask_nocontrols_grouped']};
...
for crun = 1:nrun
    for i = 2:5
        inputs{i, crun} = cell(length(filenames{i}),1);
        for j = 1:length(filenames{i})
            inputs{i,crun}(j) = cellstr(['/' fullfile(split_stem{i}{j}{1:end-1}) '/smwc1' split_stem{i}{j}{end}]);
        end
    end
end

try
    inputs{6, 1} = tiv(length(filenames{1})+1:end);
catch
    filename = tiv_filename;
    delimiter = ',';
    startRow = 2;
...
inputs{7, 1} = all_agelist(length(filenames{1})+1:end);
inputs{8, 1} = all_strengths_VBM(length(filenames{1})+1:end);
...
%% Now re-run with controls and patients as separate groups
if exist(tiv_filename)
    filename =tiv_filename;
else
    error('no tiv_file found')
...
nrun = 1;
jobfile = {'/imaging/mlr/users/tc02/vespa/scans/PNFA_VBM/tom/VBM_batch_covariate_MCI_TIV_age_patstogether.m'};
jobs = repmat(jobfile, 1, nrun);
inputs = cell(10, nrun);

stats_folder = {[outdir 'VBM_stats/factorial_full_group_vbm_TIVnormalised_agecovaried_unsmoothedmask_grouped_patstogether']};
...
for crun = 1:nrun
    ...
    for i = 2:5
        for j = 1:length(filenames{i})
            if i==2&&j==1
                inputs{3,crun}(1) = cellstr(['/' fullfile(split_stem{i}{j}{1:end-1}) '/smwc1' split_stem{i}{j}{end}]);
            else
                inputs{3, crun}(end+1) = cellstr(['/' fullfile(split_stem{i}{j}{1:end-1}) '/smwc1' split_stem{i}{j}{end}]);
            end
        end
    end
    inputs{3,crun} = inputs{3,crun}';
end
...
inputs{5, 1} = all_agelist;
inputs{6, 1} = all_strengths_VBM;
