function spm_bms_test_null_nameoutput(logbf_file,outputfname)
% TEC edit of spm_bms_test_null to allow named output file
...
j1.input      = {logbf_file};
j1.output     = {spm_file(logbf_file,'basename',['logBF_null_' outputfname])};
j1.expression = '-i1';
...
j2.mask        = {''};
j2.nsamp       = '1e6';
j2.mod_name      = {outputfname};
spm_run_bms_map(j2);

% % Display Map
% j3.file  = {fullfile(p,['BMS.mat'])};
% j3.img   = {fullfile(p,[outputfname spm_file_ext])};
% j3.thres = 0.7;
% j3.k     = 0;
% j3.scale = 0;
% spm_run_bms_vis(j3);
