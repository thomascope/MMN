function Do = spm_eeg_grandmean_megonly(S)
...
fid     = [];
for i = 1:Nfiles
    nc(i) = size(D{i}.selectchannels('MEGPLANAR'),2)+size(D{i}.selectchannels('MEGMAG'),2);
    ns(i) = D{i}.nsamples;
    fs(i) = D{i}.fsample;
...
if length(unc)~=1
    ...
    fprintf('\n')
    for i=1:length(unc)
        fprintf(['\nThose files have ',num2str(unc(i)),' MEG channels:\n',...
            fna{i}])
    end
    estr = [estr,'Data don''t even have the same number of MEG channels.\n'];
end
...
if strncmp(D{1}.transformtype, 'TF',2)
    for i = 1:Ntypes
        ...
        for j = [D{i}.selectchannels('MEGMAG'),D{i}.selectchannels('MEGPLANAR')]
        ...
        Do(1:D{1}.nchannels, 1:Do.nfrequencies, 1:Do.nsamples, i) = d;
    ...
    for i = 1:Ntypes
        ...
        Do(1:D{1}.nchannels, 1:Do.nsamples, i) = d;
